...
async function buildTarArchive(entries: Array<{ name: string; contents: Uint8Array }>): Promise<Uint8Array> {
  const pack = tar.pack();
  ...
  for (const entry of entries) {
    await new Promise<void>((resolve, reject) => {
      pack.entry({ name: entry.name, type: 'file' }, Buffer.from(entry.contents), (err) => {
        if (err) {
          reject(err);
          return;
        }
