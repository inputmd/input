  ...
  while (i < parts.length) {
    ...
    if (c === 38 && i + 1 < parts.length) {
      ...
      if (m === 2 && i + 4 < parts.length) {
        s.fg = clampRgb(parts[i + 2], parts[i + 3], parts[i + 4]);
        i += 5;
        continue;
      } else if (m === 5 && i + 2 < parts.length) {
        s.fg = color256(parseInt(parts[i + 2], 10));
        i += 3;
        continue;
      }
    }
    if (c === 48 && i + 1 < parts.length) {
      const m = parseInt(parts[i + 1], 10);
      if (m === 2 && i + 4 < parts.length) {
        s.bg = clampRgb(parts[i + 2], parts[i + 3], parts[i + 4]);
        i += 5;
        continue;
      } else if (m === 5 && i + 2 < parts.length) {
        s.bg = color256(parseInt(parts[i + 2], 10));
        i += 3;
        continue;
      }
    }
