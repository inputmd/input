...
export interface ResolveTerminalRouteEligibilityOptions {
  ...
  workspaceSessionRouteEligible?: boolean;
...
export function resolveTerminalRouteEligibility(options: ResolveTerminalRouteEligibilityOptions): boolean {
  ...
  if (options.workspaceSessionRouteEligible) return true;
  if (options.route?.name === 'new') return true;
  if (options.route?.name === 'repodocuments') return true;
  if ((options.route?.name === 'gist' || options.route?.name === 'edit') && !options.route.params.filename) return true;
  const routeEditPath = options.route ? editablePathFromRoute(options.route) : null;
