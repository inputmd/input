...
export function App() {
  ...
  const repoWorkspace = useRepoWorkspace({
    workspaceIdentity,
    gistFiles,
    currentFileName,
    currentRepoDocPath,
  ...
  const handleSessionExpired = useCallback(() => {
    ...
    setViewPhase('error');
  }, [resetRepoState, setCurrentDocumentDraft, setCurrentDocumentSavedContent]);
  ...
  const fetchDocumentForStack = useCallback(
    async (rawRoute: string): Promise<StackEntry | null> => {
      ...
      const knownPaths = getRepoMarkdownPaths();
      const wikiLinkResolver =
  ...
  const primeInstalledRepoSidebar = useCallback(
    async (instId: string, repoName: string) => {
      ...
      replaceRepoSnapshot(allFiles);
    },
    [loadRepoAllFiles, replaceRepoSnapshot],
  );
  ...
  const primePublicRepoSidebar = useCallback(
    ...
    [loadPublicRepoAllFiles, replaceRepoSnapshot],
  );

  const refreshRepoTreeAfterWrite = useCallback(
    async (options?: { invalidateTerminal?: boolean }): Promise<RepoDocFile[] | null> => {
      ...
      replaceRepoSnapshot(files, options);
      return files;
    },
    [repoAccessMode, activeInstalledRepoInstallationId, selectedRepo, loadRepoAllFiles, replaceRepoSnapshot],
  );
  ...
  const primeInstalledRepoState = useCallback(
    ...
    [installationId, loadRepoAllFiles, replaceRepoSnapshot],
  );
      ...
      try {
        ...
        if (stagedOverlayFile || renamedBaseSourcePath) {
          ...
          if (!stagedOverlayFile && renamedBaseSourcePath) {
            ...
            if (!isRepoFile(sourceContents)) {
              const markdownDirectoryIndex = findMarkdownDirectoryIndexPath(sourceContents, renamedBaseSourcePath);
              if (markdownDirectoryIndex)
                sourceContents = await getRepoContents(instId, repoName, markdownDirectoryIndex);
            }
            if (!isRepoFile(sourceContents)) throw new Error('Expected a file');
            const sourceBytes = sourceContents.content ? decodeBase64ToBytes(sourceContents.content) : new Uint8Array();
            binary = isLikelyBinaryBytes(sourceBytes);
            decoded = binary ? '' : new TextDecoder().decode(sourceBytes);
            binaryUrl = repoRawFileUrl(instId, repoName, sourceContents.path);
            sourceSha = sourceContents.sha;
          }
          ...
          setCurrentRepoDocSha(sourceSha);
          setSharedRepoInstallationId(null);
          setCurrentGistId(null);
          setGistFiles(null);
          setCurrentFileName(path);
          const knownMarkdownPaths = getRepoMarkdownPaths();
          const wikiPaths = knownMarkdownPaths.includes(path) ? knownMarkdownPaths : [...knownMarkdownPaths, path];
          const redirected = presentLoadedFileContent({
            ...
            onBinaryEditRedirect: () => {
              navigate(routePath.repoFile(owner, repo, path));
            },
        ...
        let knownMarkdownPaths = getRepoMarkdownPaths();
      ...
      void (async () => {
        try {
          if (repoAccessMode === 'installed' && activeInstalledRepoInstallationId && selectedRepo) {
            const files = await loadRepoAllFiles(activeInstalledRepoInstallationId, selectedRepo);
            if (active) replaceRepoSnapshot(files, { invalidateTerminal: false });
            return;
          }
  ...
  }, [clearAuthDataCaches, navigate, resetReaderAiModelsForAuth, resetRepoState]);
  ...
  const applyInstalledRepoWorkspaceMutationToTerminal = useCallback(
    (mutation: {
      deletes?: string[];
      ...
      if (mutation.deletes?.length) {
        removeRepoFileContents(mutation.deletes);
      }
      if (mutation.renames?.length) {
        applyRepoContentRenames(mutation.renames);
      }
      for (const file of mutation.creates ?? []) {
        setRepoFileContent(file.path, file.content);
      }
    ...
    async (options?: { content?: string; title?: string }): Promise<CommitResult | null> => {
      ...
      try {
        ...
        if (editingBackend === 'repo' && currentRepoDocPath && repoAccessMode === 'shared' && currentRepoRef) {
          ...
          const knownMarkdownPaths =
            refreshedFiles?.filter((file) => isMarkdownFileName(file.path)).map((file) => file.path) ??
            getRepoMarkdownPaths();
          renderDocumentContent(content, currentRepoDocPath.split('/').pop() ?? null, currentRepoDocPath, undefined, {
            currentDocPath: currentRepoDocPath,
          ...
          if (currentRepoRef) {
            ...
            if (refreshedCurrentFile?.sha) {
              updatePostSaveVerification({
                ...
                path: currentRepoDocPath,
                expectedSha: refreshedCurrentFile.sha,
              });
            }
            commitResult = {
              kind: 'repo',
  ...
  const commitInstalledRepoTerminalOverlay = useCallback(async () => {
    ...
    try {
      ...
      if (currentRepoDocPath && currentRepoRef && refreshedCurrentFile?.sha) {
        updatePostSaveVerification({
          routeKey: routeKeyForRepo(currentRepoRef.owner, currentRepoRef.repo, currentRepoDocPath),
          status: 'verifying',
          kind: 'repo',
          installationId: instId,
          repoFullName: repoName,
      ...
      const message = isRepoWriteConflictError(err)
        ? "Save failed: your workspace changes weren't persisted because the repo changed upstream. Refresh and retry."
        : err instanceof Error
          ? err.message
          : 'Failed to save';
      void showAlert(message);
    ...
    async (filePath: string) => {
      try {
        ...
        if (store.kind === 'gist') {
          ...
          upsertRepoFile(createdFile);
          setRepoFileContent(createdFile.path, '');
          setHasUnsavedChanges(false);
          if (selectedRepoRef) {
  ...
  const handleOpenDailyNote = useCallback(async () => {
    ...
    const existingDailyNote =
      currentGistId !== null ? Boolean(gistFiles?.[dailyNotePath]) : hasRepoSidebarPath(dailyNotePath);
    ...
    async (directoryPath: string) => {
      try {
        ...
        if (store.kind === 'gist') {
          ...
          upsertRepoFile(createdSeedFile);
          setRepoFileContent(createdSeedFile.path, '');
          setHasUnsavedChanges(false);
        }
  ...
  const handleUploadFileToSidebar = useCallback(
    async (file: File, targetFolderPath: string) => {
      ...
      try {
        ...
        upsertRepoFile(createdFile);
        setRepoFileContent(createdFile.path, new TextDecoder().decode(bytes));
  ...
  const handleDeleteFolder = useCallback(
    async (folderPath: string) => {
      ...
      const repoTargets = listRepoSidebarFilesInFolder(folderPath);
      ...
      try {
        ...
        if (store.kind === 'gist') {
          ...
          const baseRepoTargetPaths = Array.from(
            new Set(
    ...
    async (oldPath: string, newPath: string) => {
      ...
      try {
        ...
        if (store.kind === 'gist') {
          ...
          const paths = listRepoSidebarFilesInFolder(oldPath);
          if (paths.length === 0) {
            dismissToast(renameToastId);
            return;
          }
          ...
          const sourcePaths = new Set(renames.map((rename) => rename.from));
          const conflictingDestination = renames.find(
            (rename) => rename.to !== rename.from && hasRepoSidebarPath(rename.to) && !sourcePaths.has(rename.to),
          );
          if (conflictingDestination) {
            throw new Error(`Rename conflict: "${conflictingDestination.to}" already exists.`);
          }
          const baseRenames = renames
            .map((rename) => {
              const baseSourcePath = resolveRepoBasePath(rename.from);
  ...
  }, [applyInstallationSessionState, navigate, resetRepoState, showConfirm]);
  ...
  const repoSidebarBinding = useRepoSidebarBinding({
    ...
    canViewOnGitHub:
      currentGistId !== null || selectedRepo !== null || publicRepoRef !== null || currentRouteRepoRef !== null,
  ...
  }, [editPreviewEnabled, editingBackend, currentRepoDocPath, getRepoMarkdownPaths, currentFileName, gistFiles]);
  ...
  const repoEditorBinding = useRepoEditorBinding({
    baseProps: {
    ...
    canSave: canSaveCurrentEdit,
    floatingActions: showReaderAiFloatingActions
      ? {
          visible: true,
      ...
      >
        {showSidebar && (
          <>
            ...
            <Sidebar key={sidebarWorkspaceKey} {...repoSidebarBinding.props} />
            <div
